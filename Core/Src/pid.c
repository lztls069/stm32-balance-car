...
/* 2026-09-25 最终结论（用户实物复核）：gyroz 正 = 右转（旧笔记正确），
 * 阻尼侧是原版这一侧（Sign=+1 配 Kd>0），方向由 Kp/Kd 的相对符号决定，
 * 要"按右键右转"须两者异号，故 Kp 取负。Kp 只作用于 Kp*Target_turn，
 * 静止时 Target_turn=0，因此翻 Kp 不影响"平衡时不抖、不自转"的表现。 */
